...
def clean_logs(data_file,template_file,log_type):
    ...
    log_clean.EventId = log_clean.EventId.map(events)
    
####Create column with blk_id extracted from ParameterList if HDFS
    if log_type == "HDFS":
        #Parameters
    ...
    log_clean.to_csv("./Parsing/cleanlogs/"+log_type+"_structured_clean.csv",sep=",",index=False)
    print("./Parsing/cleanlogs/"+log_type+"_structured_clean.csv"+" created.")
...
def label_log(data_file,label_file,log_type="HDFS"):
    ...
    print("./Parsing/cleanlogs/"+log_type+"_labelled.csv"+" created.")
